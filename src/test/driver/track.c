...
main(int argc, const char *argv[])
{
  ...
  if (cdio_have_driver(DRIVER_BINCUE)) {
      cdObj = cdio_open("/src/external-vcs/savannah/libcdio/test/data/cdda.cue", DRIVER_UNKNOWN);
  } else if (cdio_have_driver(DRIVER_CDRDAO)) {
      cdObj = cdio_open("/src/external-vcs/savannah/libcdio/test/data/cdda.toc", DRIVER_UNKNOWN);
  } else {
